# controller.py
import time
import requests

import torch

from config import CFG
from core_rl import Net, get_latest_market_state
from fusion import fusion_modulator
from telegram_utils import invia_segnale_telegram
from logging_utils import registra_operazione


class DQNWrapper:
    def __init__(self, model: Net):
        self.model = model
        self.model.eval()

    def predict_action(self, state_flat_with_sentiment):
        """
        state_flat_with_sentiment: lista lunghezza 25 (5 giorni * 5 features)
        """
        x = torch.tensor(state_flat_with_sentiment, dtype=torch.float32).unsqueeze(0)
        with torch.no_grad():
            q = self.model(x)
        a = int(torch.argmax(q, dim=1).item())
        return {0: "SELL", 1: "HOLD", 2: "BUY"}[a]


class MasterController:
    def __init__(self, ticker: str, dqn: DQNWrapper, sentiment_module):
        self.ticker = ticker
        self.dqn = dqn
        self.sentiment_module = sentiment_module
        self.is_running = False

        # Paper trading
        self.capital = CFG.PAPER_START_CAPITAL
        self.position = 0.0  # quantità BTC simulata

    def fetch_full_state(self):
        # 1) stato tecnico (20) + prezzo reale
        tech_flat, last_close = get_latest_market_state(self.ticker, CFG.WINDOW_SIZE, CFG.SCALER_PATH)

        # 2) sentiment (un valore)
        sent, rel = self.sentiment_module.get_current_sentiment(self.ticker)
        sent = float(sent)
        rel = float(rel)

        # 3) costruiamo stato full per la rete: 5 giorni * 5 feature = 25
        # Sentiment lo replichiamo su ciascun giorno della finestra: (5 valori)
        sentiment_block = [sent] * CFG.WINDOW_SIZE

        # tech_flat è 5*4=20. Va trasformato in 5 righe da 4, e aggiungere 5° colonna (sentiment)
        full = []
        for i in range(CFG.WINDOW_SIZE):
            row4 = tech_flat[i*4:(i+1)*4]
            full.extend(row4 + [sentiment_block[i]])

        return full, last_close, sent, rel

    def run_cycle(self):
        state_full, last_close, sent, rel = self.fetch_full_state()

        # decisione tecnica del DQN
        tech_action = self.dqn.predict_action(state_full)
        tech_signal = {"SELL": -1.0, "HOLD": 0.0, "BUY": 1.0}[tech_action]

        # fusione con sentiment
        fused = fusion_modulator(tech_signal, sent, rel)

        # azione finale da fused
        if fused > 0.2:
            action = "BUY"
        elif fused < -0.2:
            action = "SELL"
        else:
            action = "HOLD"

        # paper trading semplice (no commissioni)
        
        if action == "BUY":
            # compra 10% del capitale
            spend = self.capital * 0.10
            if spend > 0:
                qty = spend / last_close
                self.position += qty
                self.capital -= spend

        elif action == "SELL":
            # vende tutta la posizione
            if self.position > 0:
                self.capital += self.position * last_close
                self.position = 0.0
        registra_operazione(action, last_close, self.capital, self.position)

        # telegram solo se BUY/SELL (o modifica come vuoi)
        if action != "HOLD":
            msg = (
                f"🤖 *Segnale IA*\n"
                f"Asset: *{self.ticker}*\n"
                f"Prezzo: `${last_close:,.2f}`\n"
                f"Sentiment: `{sent:+.3f}` (rel {rel:.2f})\n"
                f"Azione: **{action}**\n"
                f"Capitale: `${self.capital:,.2f}` | Posizione: `{self.position:.6f}` BTC"
            )
            invia_segnale_telegram(msg)

        print(f"[LIVE] {action} @ {last_close:.2f} | cap={self.capital:.2f} pos={self.position:.6f} sent={sent:+.3f}")

    def run_safe_cycle(self):
        try:
            self.run_cycle()
        except requests.exceptions.ConnectionError:
            print("⚠️ Connessione giù. Riprovo tra 60s...")
            time.sleep(60)
        except Exception as e:
            invia_segnale_telegram(f"🚨 CRASH BOT: {str(e)}")
            self.is_running = False
            raise
